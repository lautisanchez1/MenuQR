...
#   IMAGE_TAG=v1 bash scripts/deploy-backend.sh
#   SKIP_MVN=1 bash scripts/deploy-backend.sh    # solo docker (jar ya compilado)
#   SKIP_DEPLOY=1 bash scripts/deploy-backend.sh # solo build + push
set -euo pipefail
...
ALB_DNS="$(terraform -chdir="${TF_DIR}" output -raw backend_alb_dns_name)"
...
echo "  Health:  http://${ALB_DNS}/q/health/ready"
echo ""